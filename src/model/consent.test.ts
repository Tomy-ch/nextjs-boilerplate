...
describe("parseConsentState", () => {
  ...
  it("同意の綴りを、任意の用途を許した意思として読む", () => {
    expect(parseConsentState(CONSENT_CHOICE.granted)).toEqual({
      status: "decided",
  ...
  it("拒否の綴りを、任意の用途を許さない意思として読む", () => {
    expect(parseConsentState(CONSENT_CHOICE.denied)).toEqual({
      status: "decided",
  ...
  it("知らない綴りは、意思として読まずに未選択へ倒す", () => {
    expect(parseConsentState("all")).toEqual({ status: "unset" });
  });
