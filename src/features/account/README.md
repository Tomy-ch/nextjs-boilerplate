...
（追加不要）account/README.md の運用節には既に同内容の記述がある——「合成はフロント側で行います。編集画面が要る「自分の情報」と「都道府県マスタ」は互いに独立で、並べるだけで足ります。ドメインの計算を挟まない合成をバックエンドへ持たせると、画面の都合で契約が1本増えます(screens.md §1)。」。コード側はこの決定を編集せずに覆せない前提（バックエンド設計判断）を再掲しており、残すのは並行取得というこの宣言固有の制約だけでよい。
