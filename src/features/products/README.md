...
- この画面専用の表示（一覧・カード・待機表示・失敗表示・検索欄）
...
| ファイル | 役割 |
| --- | --- |
| `products-page-content.tsx` | 取得と組み立て。待機表示の境界がここに掛かる |
| `product-list.tsx` | 並べる。空のときの案内も持つ |
| `product-card.tsx` | 1 件の見た目 |
| `product-list-skeleton.tsx` | 待機表示 |
| `product-list-error.tsx` | 失敗表示 |
| `product-search.tsx` | キーワード検索。URL を書き換える client island |
| `product-pagination.tsx` | cursor 方式のページ送り |
| `product-query.ts` | URL の検索条件を取得条件へ直す |
