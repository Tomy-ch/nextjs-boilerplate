...
- **絞り込みの部品群には `"use memo"` を付けています**（[0042](../../../docs/adr/0042-react19-rendering-api.md)
  決定 4）。下書きは 1 つの state であり、1 打鍵・1 チェックがこの帯・脇・overlay の部品すべてへ及びます。
  一覧本体は `children` として受け取るので巻き込まれず、印が要るのはこの部分木だけです。付けた費用は
  `/products` の初期 JS +2.6 KB で、共有 chunk は増えません。**外しても実装は成立します** —— 印は
  `next.config.ts` の `reactCompiler` が無ければ無効な文字列式として残るだけです
