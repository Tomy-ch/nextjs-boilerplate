...
describe("ProductLoadMoreList", () => {
  ...
  it("読み込み済みの商品を並べる", () => {
    render(<ProductLoadMoreList items={ITEMS} loadMore={EXHAUSTED} />);

    expect(screen.getAllByRole("listitem")).toHaveLength(3);
  });

  it("総数が分かれば全体の何件中かを示す", () => {
    render(<ProductLoadMoreList items={ITEMS} loadMore={IDLE} total={10} />);

    expect(screen.getByText("全 10 件中 3 件を表示中")).toBeVisible();
  });

  it("総数が分からなければ読み込み済みの件数だけを示す", () => {
    render(<ProductLoadMoreList items={ITEMS} loadMore={IDLE} />);
  ...
  it("続きがあれば末尾を見張る目印を置く", () => {
    ...
    render(<ProductLoadMoreList items={ITEMS} loadMore={IDLE} sentinelRef={sentinelRef} />);
  ...
  it("取得中は進行を伝える", () => {
    render(<ProductLoadMoreList items={ITEMS} loadMore={{ status: "loading" }} />);
  ...
  it("続きが無ければ末尾を見張る目印を置かない", () => {
    const sentinelRef = createRef<HTMLDivElement>();
    render(<ProductLoadMoreList items={ITEMS} loadMore={EXHAUSTED} sentinelRef={sentinelRef} />);

    expect(sentinelRef.current).toBeNull();
  });

  it("1 件も無ければ 0 件として空の案内を出す", () => {
    render(<ProductLoadMoreList items={[]} loadMore={EXHAUSTED} total={0} />);
  ...
  it("a11y 自動検査に違反しない", async () => {
    const { container } = render(
      <ProductLoadMoreList
        items={ITEMS}
        loadMore={{ status: "failed", onRetry: vi.fn() }}
        total={10}
      />,
    );
  ...
  it("失敗したときだけ読み直す操作を出す", () => {
    render(<ProductLoadMoreList items={ITEMS} loadMore={{ status: "failed", onRetry: vi.fn() }} />);
  ...
  it("読み直す操作を押すと続きの取得を頼む", async () => {
    const onRetry = vi.fn();
    render(<ProductLoadMoreList items={ITEMS} loadMore={{ status: "failed", onRetry }} />);
