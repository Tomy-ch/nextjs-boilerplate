...
| `order.ts` | カートのうち、この購入に載せる明細の取り出し |
...
| `confirm/ui/order-line-row/` | 再掲の 1 行。今回の購入に載らない明細は理由を添える |
...
**確定できるのは、事情の立っていない明細だけです。** 買えない明細はもちろん、値が変わっただけの
明細も外します。小計はそれらを除いた合算であり、載せると画面が見せた金額と請求額が食い違います。
外れた明細はカートに残り、利用者はカートで新しい価格を確かめ直せます。
