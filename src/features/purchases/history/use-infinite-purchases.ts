 ...
 *
 * **資格情報が切れたら、続きの失敗として扱いません。** 読み直させても同じ結果になるだけで、
 * 利用者は入り直すまで抜け出せません。送り先を決めるのは route の確定認可なので、サーバへ
 * 描き直しを頼んでその判断へ委ねます（[0079](../../../../docs/adr/0079-auth-frontend-seam.md)）。
 *
  ...
  const loadMore = useCallback(() => {
    ...
    fetchPurchaseHistoryPage(params, controller.signal)
      ...
      .catch((cause: unknown) => {
        ...
        // 資格情報が切れていたら、続きの失敗としては扱わない。取り直させても同じ結果になる。
        // 未認証のときどこへ送るかは route が既に持っているので、サーバへ描き直しを頼んで
        // その判断へ委ねる。ここで送り先を決めると、同じ決定が 2 か所に増える。
