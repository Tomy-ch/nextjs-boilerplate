...
| --- | --- | --- | --- | --- |
| `shipPurchasesAction` | `src/app/admin/shipments/actions.ts` | `ShipmentState` | 1 件でも通れば一覧を取り直す | 通った件数と通らなかった件数を両方返す |
| `deliverPurchaseAction` | 同上 | `DeliveryState` | 同上 | 拒まれた理由をその操作の隣に出す |

## テスト観点

- [ ] まとめる操作が、同じ送信に注文を並べて送る形になっている
- [ ] 途中で打ち切られた送信が、通った件数を残したまま返る
- [ ] 1 件でも通ったら一覧が取り直される
- [ ] 配達の確認がまとめられない
- [ ] 便と便の中の順序を画面が並べ直さない
