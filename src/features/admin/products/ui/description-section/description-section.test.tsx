...
describe("ProductDescriptionSection", () => {
  ...
  it("書式付きの本文を書く面として公開する", async () => {
    render(<ProductDescriptionSection initialValue="" onValueChange={noop} value="" />);
  ...
  it("書いた内容を送信へ載せる。編集面は form の値を持たないため", () => {
    const { container } = render(
      <ProductDescriptionSection initialValue="" onValueChange={noop} value="<p>いま</p>" />,
    );
  ...
  it("本文が空でも送信の欄そのものは残す", () => {
    const { container } = render(
      <ProductDescriptionSection initialValue="" onValueChange={noop} value="" />,
    );
