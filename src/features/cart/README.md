...
- カートの表示（PC で常設する領域、タブレットとスマホで被せる領域、header の点数、明細 1 行の操作）
- 明細から導ける表示値（小計）
...
| `ui/panel/` | 中身を脇に常設する領域。空なら描画しない。PC だけ |
...
- **中身の置き場所は帯で変えます**。PC では脇に常設し、タブレットとスマホでは本文へ被せます
...
- **開くかどうかは store が持ちます**。カートへ入れた直後は開いた状態にする必要があり、その操作は
