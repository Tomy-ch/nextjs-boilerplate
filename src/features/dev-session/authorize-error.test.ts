...
describe("authorizeFailurePath", () => {
  ...
  it("理由を載せて発行の画面へ戻す", () => {
    const path = new URL(
      authorizeFailurePath("/account", "tx-state", AUTHORIZE_ERROR.INVALID),
      "http://localhost:3000",
    );
  ...
  it("戻り先と対応づける値を持ち回る", () => {
    const path = new URL(
      authorizeFailurePath("/account", "tx-state", AUTHORIZE_ERROR.UNAVAILABLE),
      "http://localhost:3000",
    );

    expect(path.searchParams.get("returnUrl")).toBe("/account");
