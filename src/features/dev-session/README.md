...
- 認可の往復の途中で開かれたときに、要求と応答を対応づける値を送信へ載せること
...
| ファイル | 役割 |
| --- | --- |
| `paths.ts` | この画面のルートと、戻り先・対応づける値の受け渡しの名前 |
...
IdP ではなくこの画面になります（`env/README.md`）。**その経路で来たときは、発行の結果が
`/api/auth/callback` を通ります** —— session を置くのは向こうで、この画面は認可コードを渡すだけです。
