...
> **取り方は `adapters/server/auth/development-token.ts` が 1 か所で持ちます。** 相手は同じ作者の
> `go-boilerplate` が compose で立てる mock OIDC サーバで、標準の手順ではありません（本物の IdP で
> 使ってはならない付与方式です）。**別の mock 認証を使うなら、書き換えるのはこのファイルだけ**で、
> 画面と Server Action は「主体と接続先を渡すとトークンが返る」ことしか知りません。
