...
テストは `tablist` / `tab` / `tabpanel` の意味論と `tablist` の命名、選択中だけが `aria-selected` になること、選択中のパネルだけを表示すること、選択によるパネルの入れ替え、矢印キーでの移動、link ではなく button として公開されること、`disabled` を渡した場合に実際へ選択を弾くこと、a11y 自動検査を確認します。
