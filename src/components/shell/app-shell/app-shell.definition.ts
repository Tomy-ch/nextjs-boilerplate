 ...
 *
 * **測らずに押し付けるのは、これが決められる量だからである。** header の下に貼り付く帯のように
 * 中身の量で折り返して伸びるものは、測る以外に高さを知る方法が無い（`ResizeObserver`）。header は
 * 中身がどう変わっても高さが動かない値としてここが決め、header 自身がこの値で描く。押し付けて
 * いる限り、書き写した値と実際の描画がずれることは起こらない。
 *
