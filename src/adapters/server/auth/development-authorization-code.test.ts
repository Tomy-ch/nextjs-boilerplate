...
describe("issueDevelopmentAuthorizationCode", () => {
  ...
  it("指定を URL に読める形で載せない", async () => {
    const code = await issueDevelopmentAuthorizationCode({
      subject: "dev-admin",
...
describe("openDevelopmentAuthorizationCode", () => {
  ...
  it("発行した指定をそのまま戻す", async () => {
    ...
    expect(await openDevelopmentAuthorizationCode(code, sessionSecret)).toEqual({
      subject: "dev-user",
      role: SESSION_ROLE.user,
      expiresInSeconds: 120,
      accessToken: "token-from-idp",
  ...
  it("トークンを渡していなければ、戻した指定にも載せない", async () => {
    ...
    expect(await openDevelopmentAuthorizationCode(code, sessionSecret)).not.toHaveProperty(
      "accessToken",
    );
  });
  ...
  it("役割を書き換えたコードを拒む", async () => {
    const code = await seal(
      { sub: "dev-user", role: "superuser", expiresInSeconds: 120 },
      sessionSecret,
    );
