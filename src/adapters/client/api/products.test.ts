...
import { fetchProductListPage } from "./products";
...
describe("fetchProductListPage", () => {
  ...
  it("応答の一覧を 1 ページとして返す", async () => {
    ...
    await expect(fetchProductListPage({ first: "20" })).resolves.toEqual(payload);
  });

  it("最終ページのカーソルを null のまま持つ", async () => {
    stubFetch(200, { ...payload, nextCursor: null });

    const page = await fetchProductListPage({ first: "20" });
  ...
  it("取得条件をクエリへ載せる", async () => {
    ...
    await fetchProductListPage({ keyword: "本", after: "cursor" });
  ...
  it("打ち切り用のシグナルを fetch へ渡す", async () => {
    ...
    await fetchProductListPage({}, controller.signal);
  ...
  it("400 が返ったとき invalid-argument へ写す", async () => {
    ...
    await expect(kindOf(() => fetchProductListPage({ first: "0" }))).resolves.toBe(
  ...
  it("400 以外で失敗したとき internal へ写す", async () => {
    ...
    await expect(kindOf(() => fetchProductListPage({}))).resolves.toBe(ErrorKind.INTERNAL);
  });
  ...
  it("応答が一覧の形を満たさないとき検証の失敗を原因として残す", async () => {
    ...
    await expect(causeOf(() => fetchProductListPage({}))).resolves.toBeInstanceOf(z.ZodError);
  });
});
