...
describe("fetchAddresses", () => {
  ...
  it("BFF が返した候補をそのまま渡す", async () => {
    stubFetch(200, { candidates: [candidate], isFallback: false });

    await expect(fetchAddresses("150-0001")).resolves.toEqual({
  ...
  it("郵便番号をクエリへ載せて同一オリジンの口を叩く", async () => {
    const fetchImpl = stubFetch(200, { candidates: [], isFallback: false });

    await fetchAddresses("150-0001");
  ...
  it("クエリに載せる前に URL として安全な形へ変換する", async () => {
    ...
    await fetchAddresses("150 0001&x=1");
  ...
  it("打ち切りの signal をそのまま fetch へ渡す", async () => {
    ...
    await fetchAddresses("150-0001", controller.signal);

    expect(fetchImpl.mock.calls[0]?.[1]).toMatchObject({ signal: controller.signal });
  });

  it("該当が無いとき空の候補を返す", async () => {
    stubFetch(200, { candidates: [], isFallback: false });

    await expect(fetchAddresses("999-9999")).resolves.toEqual({
      candidates: [],
      isFallback: false,
    });
  });
  ...
  it("応答が想定の形でないとき投げずに空の候補を返す", async () => {
    stubFetch(200, { candidates: [{ prefecture: "東京都" }], isFallback: false });
  ...
  it("入力が続いて打ち切られたとき投げずに空の候補を返す", async () => {
    ...
    await expect(fetchAddresses("150-0001", controller.signal)).resolves.toEqual({
      candidates: [],
      isFallback: false,
    });
  });
