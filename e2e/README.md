...
`RUN` を渡すと、その実行の `e2e-diff` を作業ツリーへ落として隣のポートで配る（`gh` が要る）。
待ち受けるポートは `E2E_REVIEW_PORT`（既定 `3200`）。`make e2e` が使う `3100` とも開発サーバの
`3000` とも別にしてある。見終わったら Ctrl-C で止める。
...
### URL は絵を固定するところまで指定する

同じ URL には同じ応答が返る（[`mocks/stable-responses.ts`](../mocks/stable-responses.ts)）。裏を返すと、
**URL で決まらないものが絵に出ていれば、そこが基準画像との食い違いになる。**動的区間の ID を宣言で
固定しているのはそのためで、同じ理由で**実時計から組み立てられる値も URL で固定する** —— 期間を省いた
`/admin/analytics` は「今日」を実時計から求めるので、基準画像は撮った日にしか一致しない。

**ブラウザ側の時計を固定しても届かない。**描いているのはサーバであり、story 単位の側が使う手立て
（[`vrt/lib/clock.ts`](../vrt/lib/clock.ts)）はページの中の `Date` しか差し替えない。宣言する URL で
条件を名指しするのが、この撮影で取れる唯一の手立てである。
