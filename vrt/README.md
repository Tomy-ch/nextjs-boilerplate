...
`RUN` を渡すと、その実行の `vrt-diff` を作業ツリーへ落として隣のポートで配る（`gh` が要る）。
落とせなくても止まらない —— artifact は保存期間を過ぎれば消えるが、そのときに見たいのは
「いまのブランチがどう描くか」で、それはサーバさえ立てば見られる。

待ち受けるポートは `VRT_REVIEW_PORT`（既定 `6106`）。開発用の Storybook（`6006`）を避けてある。
artifact を配るのはその次の番号。塞がっていれば手前で止まるので、そのときは空いている番号を渡す。
...
> 画素を判断する面は置き場の compare ビューのままである。
...
分けない。story は表に出ていたものだけ、画面（[e2e/README.md](../e2e/README.md)）は全数が対象で、
両方が 1 つのコミットとして置き場へ入る。**compare ビューにこの PR の見た目の変化が 1 箇所に
揃う**のはそのためで、承認する人が見るのはその 1 つである。
...
- 撮るのは [`lib/themes.ts`](lib/themes.ts) の `SHOT_THEMES` ＝ **light だけ**。明るい面を選ぶのは、
  基準画像を人が画像として承認するため（判断は置き場の compare ビューで行い、その画面自体が
...
- **どの story からも参照されない基準画像が置き場に残っていないことも見る**。story を消す・
  ...
  **孤児が出たときの撮り直しは範囲を絞らない** —— 絞った撮り直しは撮ったぶんを上書きするだけで
  孤児を消さないためで、全数のときだけ撮る前に story の区画を空にする（`vrt clear-stories`）。
...
| 並列実行時のラスタライズの丸め（±1） | 色差の下限で切る（`threshold: 0.01`） |
...
低コントラストの退行がここをすり抜けるため、`0.01`（上限 2.6）を置いている。外れるのは
**同じ入力でも実行ごとに ±1 だけ揺れる画素**で、これは全 story を 4 worker で回すときにだけ出る
（story を絞って撮ると 2 回の撮影はバイト一致する）。この幅を超える差は残らず数える。
...
| `tmp/vrt/verified-inputs.sha256` | この入力で**比較が通った** | `make vrt` | 追跡外。CI は cache で持ち回る |
