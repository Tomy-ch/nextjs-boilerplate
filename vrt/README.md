...
CI は判定だけを行い、基準画像を書き換えない。差分が出た PR では
`vrt-diff` artifact に expected / actual / diff の 3 枚が入るので、それを見て

- **退行なら実装を直す**
- **意図した変更なら `make vrt-update` を回し、更新後の基準画像をコミットする**

`contents: write` を持つ job に基準画像を書かせると、承認の実体が「CI が直した」に変わる。
量産の最中に効く gate は、差分を人が引き受ける形でしか保てない。
