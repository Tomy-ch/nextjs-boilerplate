...
**一覧する口が無い参照も、同じ表が受け持ちます。** 購入ステータスは `id` と業務キーと名称を 1 つの
object で持ち、名称が解決済みで載るため backend に一覧を返す理由がありません。値域を宣言する場所が
契約の外にしか無いので、表がそれを宣言します。アプリ側にも同じ業務キーの転記がありますが
（`src/model/purchase/purchase-status.ts`）、モックは backend の代役なので**読み込まずに書き写します**。
アプリの転記から作ると、転記そのものがずれていても両方が同じだけずれ、確かめる手立てが無くなります。
2 つの宣言が一致していることは `references.test.ts` が見ます。
