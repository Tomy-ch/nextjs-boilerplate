...
//
// **これを `setupFiles` へ戻さないこと。**interception を立てる費用は 1 ファイルあたり約 480ms で、
// 全ファイルへ掛けると Vitest の setup がテスト本体より長くなる。HTTP 境界を持つのは
// `adapters` の API クライアントと Route Handler だけ（[0090](docs/adr/0090-testing-strategy.md)）で、
// それ以外は module 境界を `vi.mock` で差し替える。読み込まないファイルは、宛先を名指しで落とす
// 番人が `vitest.setup.ts` 側に立つ。
//
