...
**2 段が見分けられるかは OS が持つ太さ次第で、環境によっては見分けられません。** 同じ文字列を各段で描いて
インクの量を比べた実測（macOS / Chromium）:
...
つまり **`emphasis` と `strong` の 2 段は、どちらの系統でも保証できません**。游ゴシックでは利用者向けの
700 / 800 も、管理の 500 / 600 も潰れます。どの環境でも残るのは**本文（400）と、それより強い段との差**
だけです。段を名前で持っているのはこのためで、**部品を直さずに宣言だけで戻せます** —— 2 段が全ての環境で
要ると決めたときが、本文へ Web フォントを戻す理由になります（[0051](../docs/adr/0051-styling-system.md) §5）。
