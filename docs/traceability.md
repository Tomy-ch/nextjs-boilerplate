...
**節が 21、規約が 250 件。**
うち 18 節が節頭で機械の手段を名乗り、33 件の規約が自分で判定を述べる。
...
| 節 | 規約 | 判定 |
| --- | --- | --- |
| [描画とキャッシュ](rules.md#rendering) | `use cache` の内側の `fetch` に個別のキャッシュ指定（`cache` / `next.tags`）を置かない。 | 寄せられる |
| [描画とキャッシュ](rules.md#rendering) | 捨てるのは、その mutation が変えたデータを実際に描いている route だけにする。 | 寄せられる |
| [描画とキャッシュ](rules.md#rendering) | タグは `<資源>` と `<資源>:<識別子>` の 2 段だけを使う。 | 一部寄せられる |
...
| [レイアウトと帯](rules.md#layout) | z-index は Tailwind の段階値（`z-10` / `z-20` …）だけを使う。 | 寄せられる |
| [コメントと文書](rules.md#comments) | 名前の付いた関数には TSDoc を書き、タグを次の規則で揃える。 | 寄せられる |
...
| [生成物と補助スクリプト](rules.md#generated) | シェル変数を全角文字の直前に裸で置かない。 | 寄せられる |
