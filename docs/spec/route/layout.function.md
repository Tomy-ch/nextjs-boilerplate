...
**この経路は中継を通らない。** タグマネージャは各タグを配信元と直接喋らせる仕組みで、
`/api/telemetry` の伏せ字が掛かる範囲の外にある（[0082](../../adr/0082-client-observability.md) 禁止事項の
唯一の例外）。**容器へ何を入れるかが、そのまま何が外へ出るかになる。**

容器 ID を宣言した配備は、配信ヘッダが緩むことも同時に受け入れる —— `script-src` / `connect-src` /
`img-src` が Google の origin を許し、`Cross-Origin-Embedder-Policy` が降りる
（[0111](../../adr/0111-csp-security-headers.md) §5）。
