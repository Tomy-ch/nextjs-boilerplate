...
**分類と状態は 1 つだけ選べる。** 契約は並びで受け取るが、管理の一覧は「全部」か「その 1 つ」を
見比べる場所で、複数を混ぜた結果は行がどの条件で並んでいるかを読み取れなくする。載せる値が 1 つ
でもキー名は契約のものを使い、送る側で読み替えない。

**マスタ行を指すのはコードであり、UUID ではない。** 契約が絞り込みで受け取るのはコードで、UUID を
取る口は非推奨として残っているだけである。両方を同時に送ると 400 になるため、この画面が使う口は
コードの側だけに寄せる。

**1 つしか取らない条件に複数の値が届いたら、先頭だけを読む。** URL は利用者が直接編集できる。
どれを採るかを読む側ごとに決めると、同じ URL が画面の場所によって違う条件に見える。
