...
**数えられなかったときは消す。** 薄いまま残すと、いまの条件の件数として読まれる。件数が分からない
だけで確定は妨げないため、押せる状態は変えない。
...
**入力欄の下に置いたまま、絞り込みの器の下端へ貼り付く。** 分類が増えるほど入力欄は縦に伸び、
末尾に置いたままだと選んでいる途中で見えなくなる。器が画面に収まるあいだは入力欄のすぐ下に見え、
収まらなくなると器の送りに追従して下端に残る。**つまみは置かない** —— 脇に領域を持てる幅にしか
出ない器で、入力欄そのものがすぐ上にある。
...
| 幅 | 絞り込みの居場所 | 確定 |
| --- | --- | --- |
| `lg` 以上 | 本文の脇に常設（読み進めても残る） | 入力欄の下。器の下端に貼り付く |
