...
挟むかどうかの判断は変わらない。overlay の側は開くたびに組み立て中の条件を一覧の状態へ戻す
（閉じたときの中途半端な選択が次に開いたとき残っていると、表示されている一覧と入力欄が食い違う）。
...
**hydration を必要としない形にする。** 打鍵ごとに反映する必要が無く、送信結果は URL に載るため、
島にしても得るものがない。JavaScript が動かない環境でも送信できる。

**送信時に、絞り込みと並び替えを引き継ぐ。** この形の form は送信時に URL の条件をすべて捨てる。
複数の値を持つ条件は値の数だけ引き継ぐ（1 つにまとめると選んだうちの 1 つしか送信されない）。
**読み進めた位置は引き継がない。**
