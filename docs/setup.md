...
## 5. VRT の基準画像の置き場を用意する

**サンプル破棄（手順 6）より先に実行する。** 破棄はサブモジュールの中へ届かないため、
題材の基準画像は「自分の置き場へ張り替えたときに参照が切れる」形でしか消えない。
...
**手順 5 を済ませていること。** 済ませていない場合、`make setup-vrt-images` 自身が破棄対象なので
実行できなくなる。
