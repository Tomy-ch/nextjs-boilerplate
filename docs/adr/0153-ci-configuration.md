...
- ワークフローは関心事ごとに分ける(go 翻案)。本リポの CI job:
  ...
  - **Actions 定義の静的解析**: 上の 2 つが原理的に見られない観点を **zizmor** で検査する(`make actions-zizmor`)。actionlint も `actions-shellcheck` も、shellcheck へ渡す前に `${{ … }}` を行数だけ保つプレースホルダへ潰す。潰した時点で式は「ただの語」になり、**クオートの有無という script injection の本質が解析対象から消える** —— `run: echo ${{ github.event.issue.title }}` は攻撃者が内容を決められる値をシェルの解釈前に展開するが、shellcheck を通す設計では捕まえられない。正しい形は `env:` 経由で環境変数へ渡し `"$VAR"` で参照することであり、検査はその形へ誘導する。走査対象は workflows と composite action の両方。**落とすのは high の所見だけ**とし、medium 以下は出力に残してゲートには使わない(所見の総量ではなく、実行前に止めるべきものだけを止める)。**hook も CI も `--offline` で走らせる** —— オンラインでしか成立しない監査(impostor-commit 等)を CI 側にだけ足すと、上記 4 の hooks mirror CI が崩れて手元で再現できない失敗が PR にだけ現れる。tag 付け替えの検知は下記 3 の `resolve` が fail-closed で担う。抑制は `.github/zizmor.yml` に理由付きで宣言し、**設定は `--config` で明示的に渡す**(自動探索が外れても「所見ゼロ」ではなく「抑制が効かない」形で現れるため、外れたことに気付けない)。ワークフロー・PR コメント・required check は上の 3 つと同じく `actions-lint` に同居させる(問うているのは同じく「これから CI で実行する Actions の定義が正しいか」)
