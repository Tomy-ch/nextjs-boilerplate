...
- ワークフローは関心事ごとに分ける(go 翻案)。本リポの CI job:
  ...
  - **生成物 drift ゲート**(型生成。[0072](0072-api-type-generation.md))/ **カバレッジゲート**(97.5%。[0090](0090-testing-strategy.md))。カバレッジの **PR レポート**([0090](0090-testing-strategy.md) が B9 へ引き渡した項)は **octocov** で出力する。ほかの検査ログは下記 5 の PR コメント upsert 基盤で出力する
