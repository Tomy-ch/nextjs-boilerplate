...
| **W8** | `make actionlint` に shellcheck 個別の PATH ガードを置かない | [0153](0153-ci-configuration.md) 1 / [0003](0003-version-manager.md) | actionlint と shellcheck の**供給経路が分かれたとき** — 片方が `mise.toml` の管理から外れる、または mise 以外の経路での導入を許容したとき。現状は同一の `[tools]` から同一の activate で PATH に載るため、先行する actionlint のガードが両方を覆っている。**「shellcheck が無い環境で検査範囲が縮んだ」ことは条件にならない** — それは PATH の問題であり、ガードではなく PATH で直す |
