...
| **W21** | `eslint-plugin-security` の推奨プリセットを当てず、0 件を保てる 9 規則だけを有効にする | [0110](0110-security-operations.md) 3 / [0002](0002-formatter-linter.md) | 落とした 5 規則のいずれかが**形ではなく実体を見るようになったとき** — 例えば `detect-unsafe-regex` が星の高さではなく実際の後戻り計算量で判定するようになれば、`/^\d+(\.\d+)?$/` は鳴らなくなり 0 件を保てる。**「SAST の層が薄い」ことは条件にならない** — ReDoS と path traversal は Opengrep / CodeQL / njsscan が引き続き担い、そちらは所見を code scanning へ送る層なので 0 件 baseline を要求されない |
| **W22** | SonarQube Cloud を required check に登録しない | [0110](0110-security-operations.md) 3 / [0153](0153-ci-configuration.md) 5 | **原則としてこの決定は動かない**。動かせるのは、この検査が外部アカウントを要さなくなったときだけである。**「このリポジトリでは常に緑だから」は条件にならない** — 登録が壊すのは fork 先であり、しかも壊れ方は「必須チェック待ちで全 PR が止まる」という、コードでは直せない形になる |
