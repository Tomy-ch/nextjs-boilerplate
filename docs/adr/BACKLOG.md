...
| **W12** | 強調の 2 段(`emphasis` / `strong`)が出ない環境を受け入れる | [0051](0051-styling-system.md) §5 | 2 段の差が**全ての対応環境で要る**と決めたとき — 実測(macOS / Chromium)では游ゴシックが 400 とそれ以外しか描き分けず、`system-ui` も 500 と 600 が同じになるため、和文を OS 同梱の書体へ委ねている限り 2 段は保証できない。**戻す手段は本文へ和文 Web フォントを置くことだけ**で、同梱サンプルの実測で CSS が 25 KB から 158 KB へ戻る。**「ある環境で 2 段が見分けられない」ことは条件にならない** — それは受け入れた内容そのもの。段を semantic の名前で持っているため、決め直しても部品は触らない |
