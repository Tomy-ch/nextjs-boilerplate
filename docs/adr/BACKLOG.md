...
| **W45** | 軽い画面を TBT で締めた行を `performance-budget.yaml` の `screens` に置かない(締める側は LCP だけで書く) | [0101](0101-performance-budget.md) 3 | **割った台ごとに測っている床(`floor`)が、報告だけでなく判定の補正に使われるようになったとき。** TBT は機械の速さがそのまま乗数として効き、器しか持たない画面ほどそれを素直に写すため、いま細い線を引くと遅い実行のたびに床が鳴り、床を見て判別するという役目そのものを失う。補正が入れば、締めた線は機械の速さから切り離される。**「軽い画面の退行を捕まえたい」ことは条件にならない** —— その目的は LCP を締める行が既に果たしており、TBT で二重に引く必要は無い |
| **W46** | 弧の反字体の揺れ(実測 ±3)を story の側で潰さない | [0091](0091-test-verification-methods.md) | **同じ入力の 2 回の撮影がバイトまで一致するようになったとき** —— ラスタライズが並列度に依存しなくなるか、比較が画素の色差ではなく形で判定できるようになったとき。実測では滑走子の円弧やバッジの丸みが 15 × 8 画素以内・最大の差 2 で動き、比較の下限(`threshold: 0.02`)の内側に収まる。**潰す先は弧そのもの**なので、story 側に打つ手が無いことがやらない理由である。守りは撮る側にあり、撮り直しを報告された集合に限ることで揺れが基準へ焼き付くのを止めている。**「撮り直しの差分に説明の付かない行が出た」ことは条件にならない** —— それは全数の撮り直しが起きたということで、そちらの経路を塞ぐ話である |
| **W47** | `AppShell` の header の高さを測らず、定数として押し付ける | [0026](0026-layout-shell-mount.md) | **header の高さが中身で変わる形になったとき** —— 導線が折り返す、行が可変になる等で「決められる量」でなくなったとき。それまでは押し付けている限り書き写した値と描画がずれず、測る形は SSR と初回描画のあいだ 0 を配るため、知り得る量を知るまで間違っている仕組みに置き換えることになる。**「同じ画面で帯の高さは測っているのに不揃いだ」ことは条件にならない** —— 帯は中身の量で折り返すので測る以外に知る方法が無く、種類の違う量である |
...
| **W49** | 生成物を bot が commit する仕組み(`auto-generate-docs`)を持たない。生成物の陳腐化は drift 検査で赤くする | [0072](0072-api-type-generation.md) / [0141](0141-portal-operations.md) | **生成物が、人の手で追随できない量や頻度になったとき。** 上流のそれは DB を立てて schema から docs を作り、`contents: write` で自動 PR を開く形で、本リポジトリの生成物(OpenAPI 由来の型・token・portal)はいずれも `gen-drift` / `tokens-drift` / `shadcn-drift` が**落とす**側で守っている。書き込む権限を持つ workflow を増やさずに済むこと自体が、この形を選ぶ理由でもある。**「生成物が増えたこと」は条件にならない** —— 増えても赤くして人が回せる限り、drift 検査の側で足りる |
