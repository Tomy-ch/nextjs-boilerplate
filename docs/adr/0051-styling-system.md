...
### 5. 和文の本文書体 = OS 同梱の書体へ委ねる(Web フォントは銘と管理面に限る)
...
| ④ 用途を限る | 銘・見出しだけ Web フォント、本文は OS 同梱 | **採用**。①と組で使う |

#### この repository の決定

- **利用者向けの面の本文は OS 同梱の書体へ委ねる。** ここが全画面に効き、`404` やログインのように
  中身を持たない画面まで費用を払っていた
- **銘(ラテン)と等幅は Web フォントのままとする。** ラテンはスライスが数個で、費用が桁で違う
- **系統ごとに本文書体を替える設計は維持する**([0045](0045-fonts-and-images.md))。ただし**その書体は
  その系統の面からのみ読み込む** —— 管理面の書体を root layout から読むと、管理を開かない利用者まで
  宣言を読まされる。実測で、この 1 点の切り離しだけで全画面 −0.6 秒(gzip 134 KB)だった
- **代償は、強調の段が OS の持つ太さに縛られること。** ヒラギノ角ゴシックのように W0〜W9 を持つ環境では
  2 段が出るが、Regular と Bold しか持たない環境では潰れる。段を semantic の名前で持っているので、
  戻すときに部品は触らない
- fork 先が和文の Web フォントを本文へ戻すときは、②(文言が確定しているなら)か、③を系統ごとの面に
  限る形で行う。**root layout へ足すのは、どの画面も開かない利用者に費用を払わせる**
