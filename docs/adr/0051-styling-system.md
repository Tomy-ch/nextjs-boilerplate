...
- **代償は、強調の 2 段がどの系統でも保証できなくなること。** 実測(macOS / Chromium、同じ文字列のインク量
  で比較)では、ヒラギノ角ゴシックは 400/500/600/700/800 のすべてを描き分けるが、**游ゴシックは 400 と
  それ以外しか区別せず**(500・600・700・800 が同じ字面)、`system-ui` は 500 と 600 が同じになる。
  したがって利用者向けの 700 / 800 も管理の 500 / 600 も潰れうる。**どの環境でも残るのは、本文(400)と
  それより強い段との差だけ**である
- **2 段が出ない環境を受け入れる。** 出る環境では出る、という扱いにする。段を semantic の名前で持って
  いるため、決め直しても部品は触らない。**戻す手段は本文へ和文 Web フォントを置くことだけ**で、それは
  この節が削った費用をそのまま払い直すことを意味する(再検討の条件は [BACKLOG](BACKLOG.md) W12)
