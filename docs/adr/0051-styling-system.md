...
- **既定手段(不変)**: モーションの既定手段は **CSS transition / animation** と **View Transitions API**(ブラウザ標準 / Next.js App Router が統合)とする。いずれもブラウザ標準機構であり、特定ライブラリに縛られない(0010 §2)。単純な hover / focus / enter・状態遷移・ページ遷移アニメーションはまずこの標準手段で書く。
...
- **`prefers-reduced-motion` の尊重を必須**とする(Tailwind の `motion-reduce:` / `motion-safe:` variant、`@media (prefers-reduced-motion)`、または Framer Motion の `useReducedMotion` フックで実装)。これは WCAG 2.x AA(WCAG 2.3.3 相当)の要請であり、**根拠水準・a11y としての強制は [0100](0100-accessibility-target.md) が所有する**。本 ADR は「モーション実装時に reduced-motion 分岐を欠かさない」という体系側の帰結だけを持ち、a11y 目標の一元管理は 0100 に委ねる(二重管理回避。下記「補足」参照)。Framer Motion を用いる場合も reduced-motion 尊重は同じく必須(退場・layout・spring も低減対象)。
...
- **Figma → CSS 変数 同期は本 ADR の射程外**。token の**体系(命名層・スケール軸)**のみを本 ADR が定め、デザインツールとの**同期方式**(Figma variables → CSS の生成/取り込み)は BACKLOG B9(未 ADR frame)へ委譲する。本 ADR で同期方式に踏み込むと B9 と矛盾しうるため、意図的に体系のみへ限定した。
...
- **`prefers-reduced-motion` 必須化の管理先**は [0100](0100-accessibility-target.md)(a11y AA)と連名になる。二重管理を避けるため、本 ADR は「モーション実装は reduced-motion 対応を欠かさない」という帰結の言及に留め、**a11y 要請としての強制・根拠水準は 0100 に一元化**する立場を採る。0100 本体は Accepted の Protected Documentation のため本 ADR 作成時点では相互参照の back-link を付さない(最終整理フェーズで付与)。
...
- [0100-accessibility-target.md](0100-accessibility-target.md)(C2)— `prefers-reduced-motion` の a11y 要請水準(WCAG AA)を所有(本 ADR の §3 の根拠 / Framer Motion 使用時も同じく必須)
