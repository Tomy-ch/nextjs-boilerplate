...
- **観点の束ねは `describe` の入れ子ではなくコメント区切りで行う**。`it` は export 名 `describe` の直下に並べ、`// ----- 正常系 -----` / `// ----- 異常系 -----` のコメントで区切る。入れ子を使わないのは、観点は 2 つしかないのに階層が 1 段深くなり、失敗時の表示が `対象 > 正常系 > ケース` と冗長になるためである。区切りコメントは runtime に影響せず、ファイルを走り読みしたときに分岐の軸が見える
