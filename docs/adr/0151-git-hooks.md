...
| 段階 | 目的 | 想定処理 | 速度目標 |
| --- | --- | --- | --- |
| pre-commit | 「壊れた diff を commit に乗せない」 | `pnpm lint:ci` (biome 完全版 = `biome.ci.jsonc` + `--error-on-warnings`。ESLint 導入後は境界検査も直列 — [0002](0002-formatter-linter.md)) / Markdown 検査 (`pnpm md-lint` = markdownlint + mermaid 構文。`*.md` が staged のときのみ) / ワークフロー検査 (`make actionlint` = 構文 + `run:` のシェル。ワークフロー / composite action の定義が staged のときのみ — [0153](0153-ci-configuration.md)) | < 5 秒 |
