...
決定を**指す側**が指し先を失わないための規律を定める。対象は [`docs/rules.md`](../rules.md) の規約と、それを指すすべての文書 —— 設計文書、画面仕様、ADR、そして集計である [`docs/traceability.md`](../traceability.md)。
...
- **錨の綴りは英語の固定語とし、見出しの文言から導かない。** 導くと、見出しを訳した時点（[0140](0140-documentation-operations.md) が v1.0.0 で canonical 言語を移す）と、言い回しを変えた時点の両方で消える。GitHub が見出しから自動で作る slug は、まさにその形をしている
