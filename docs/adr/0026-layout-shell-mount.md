...
したがって根の画面(トップ)、nav が直接指している画面(一覧)、および**線形フロー**(カート → 購入確認 → 完了)には置かない。前 2 つは nav と同じ導線を二重に置くだけであり、線形フローでは「戻れる場所」を示すことが離脱の導線になる。段の進捗は `Stepper` が持つ。

- **到達経路が複数ある画面では、辿った経路ではなくサイト構造上の階層を示す**(一覧からでも分類からでも `トップ > 商品一覧 > 商品名`)
- **置く主体は画面**である。器は口を持たない。どの階層を示すかは画面の内容(商品名など)に依存し、器が知ると画面ごとの分岐を器が抱える
