...
- 本 ADR は供給の**方針**を定める。flag の具体実装(どのソース・どの gate 粒度)は用途依存で fork 先 / 実装 PR で、本体が備えるのは供給経路と gate 述語まで。**consent はこれに当たらない** —— [0131](0131-cookie-consent.md) §1 が軽量機構そのもの(保持・バナー・ゲート・計測 id の発行)を本体同梱と定めており、供給経路だけを置く対象ではない。
