...
- **外部の認証基盤へ繋ぐ経路は、前提の連鎖に従属する。** 着手できる順は **IdP の構築 → backend が正規化されたチャレンジを返す機構 → この層の画面**であり、逆順には進められない。正規化されたチャレンジの契約が無いまま画面を書くと、契約が決まった時点で必ず書き直しになる。§8 の到達点と実装が食い違っている間も、**ADR は実装へ書き戻さない**([0140](0140-documentation-operations.md) 決定 4)。
