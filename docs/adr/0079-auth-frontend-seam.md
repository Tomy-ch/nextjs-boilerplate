...
- **加えて、利用者のブラウザを IdP の終了口へ遷移させる。** IdP 側の session を保持しているのは利用者のブラウザが持つ cookie であり、サーバから発した要求にそれは載らない —— 要求は成功を返しながら、IdP 側は何も終わらない。**この機構をここが持つ**ので、実装側は繰り返さずに本節を指す(呼び出しの連鎖に沿って同じ根拠が何度も書かれることになるため)。
...
### 8. 資格情報の機構は持たない / 認証画面の意匠は所有する

**この 2 つは別の話であり、畳むと必ず片方を落とす。** 「機構が借り物だから画面も向こうのもの」は
誤りで、本 ADR が禁じているのは**資格情報がこのアプリを通ること**だけである。
...
| --- | --- | --- |
| `/login`(所有画面) | 所有 | 所有 |
| IdP の画面(借り物) | **借り物** | **所有(供給する)** |
| IdP の終了口(借り物) | **借り物** | **所有(供給する)** |
| `/onboarding`(所有画面) | 所有 | 所有 |

**往路と復路は対称である。** ログインが `/login`(所有)→ IdP の画面(借り物)→ 戻り、で成るのと同じ
形で、ログアウトも所有画面から IdP の終了口(借り物)を経て戻る(§5)。**借り物の画面を経由すること
そのものが、この seam の形**であって、経由を避けるために資格情報を自前で受けるのは §8 が禁じる側
である。

- **資格情報を受け取る機構をアプリに持たない。** ID / パスワード / MFA コードの入力欄をこの
  リポジトリの画面へ置かない。§6 が Authorization Code + PKCE を採り、Route Handler が IdP と直接
  やりとりする以上、**資格情報がこのアプリを通る経路はそもそも無い**。通せば、通した時点で保管・
  転送・漏洩がこのリポジトリの責任範囲に入る(§1 / §3 と同じ最小露出の原則)。**IdP の選択に依存
  しない恒久の線**であり、プロバイダを替えても動かない
- **所有画面が持つのは導線だけ。** `/login` が行うのは `/api/auth/login` への遷移と、失敗したとき
  に何が起きたかを伝えることである(§6)
- **認証画面の意匠は所有する。** 借り物なのは機構であって見た目ではない。IdP のブランディング機構
  へ、このリポジトリの design token から供給する。**手で二重管理しない** —— 色や書体を IdP の管理
  画面へ書き写すと、token を動かしても借り物の画面だけが古い姿で残り、利用者から見れば同じ 1 つの
  導線の途中で見た目が変わる
- **供給できる範囲は IdP のブランディング機構が決める。** 供給しきれない部分が残ることは、意匠を
  所有しないことを意味しない —— 供給できる範囲を供給し、できない範囲を分かって残すのと、何も
  供給しないのは別である

**vendor-independent 正当性材料**([0010](0010-standards-and-non-lockin.md) §2): 資格情報を通さない
のは、**扱わないデータは漏らせない**という一般原則(最小露出)による。IdP を替えても、その IdP が
資格情報を扱う限りこの線は動かない。意匠を供給元 1 つに寄せるのは、同じ値を 2 箇所で持つと必ず
片方がずれるという、この repo が token の SSOT でも採っている一般原則である。
...
- ❌ 資格情報(ID / パスワード / MFA コード)の入力欄をこのリポジトリの画面へ置くこと(§8)
