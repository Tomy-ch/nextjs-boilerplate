...
- **`connect-src` はバックエンドの origin だけを足す。** ブラウザが往復で叩く先は BFF(`/api/*`)に限り、観測性のシグナルも中継 seam を通る([0081](0081-observability-logging.md))。OTLP を直接叩かせない。例外は購読(長寿命接続)で、BFF はこれを中継できず([0074](0074-runtime-communication-seam.md) 決定 1)ブラウザが backend へ直接開く。**購読の口は API と同じ origin に置かれる前提**なので、足す origin は検証済みの `APP_API_BASE_URL` から導く 1 つで済み、入力を増やさない。購読の口が API と別の origin に置かれるなら、ここにその origin の入力を足す
