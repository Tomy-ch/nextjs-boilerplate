...
| `develop` | 開発環境 | 次回リリースに含める変更の統合先 (デフォルトブランチ) |
...
- メジャー依存の更新 (`next` / `react` / `@biomejs/biome` 等のメジャーアップ) は他の機能変更と同じコミット・PR に混ぜない (ADR 0005 と整合)
...
- デフォルトブランチは `develop`。PR の base 既定も `develop` だが、`release/*` が開いている期間中は手動で base を `release/*` に向ける
...
- [0005-library-management.md](0005-library-management.md) — 依存ライブラリ更新 PR の粒度 (メジャー更新は別 PR)
- [0007-git-hooks.md](0007-git-hooks.md) — pre-commit / pre-push hook の運用方針
