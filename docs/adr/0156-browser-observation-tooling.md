...
**撤回条件は、CLI に生成されない道具でしか答えられない問いが実測で現れたとき。** 待機と一括入力は CLI へ生成されないが、待機が要る観測は別レーンが担うため現状は成立している。登録する場合も置き場は利用者のホーム側で、リポジトリには残さない。**「MCP のほうが呼びやすい」ことは条件にならない** —— 文脈の常時消費と、作った側へエージェント構成が降ることが対価である。
