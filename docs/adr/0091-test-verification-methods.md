...
- **承認は画像差分のレビューで行う**。基準画像を撮り直す操作(ラベル / 手元での実行)は「画素を見られる形にする」までであり、それ自体は承認ではない。差分は撮り直しのコメントが動いた画像 1 枚ずつに並べる前後のリンクで見る — 画像そのものを PR コメントへ貼らないのは、置き場が非公開のとき GitHub の画像プロキシが匿名でアクセスして 404 になるため(見る人の権限は関係ない)。置き場の compare ビューは使えない: 撮り直しは一式まるごとを置き場の根の上に積むので、共通の祖先が根になり、動いた数枚ではなく一式まるごとが「追加」として並ぶ
