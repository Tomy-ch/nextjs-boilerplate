...
- **story 全数を実ブラウザで検査する**。visual regression と同じ digest 固定コンテナ・同じ story 列挙に相乗りし、追加のランナーは入れない(§3)。**実ブラウザであることが本質** — jsdom には描画も色計算も無く `color-contrast` を検査できないため、component 層では同ルールを無効にしている。story 側は light / dark を実描画するので、テーマの切り替えでしか出ないコントラスト違反まで届く
...
- **無効化するルールは理由と撤去条件を添えて 1 箇所へ宣言する**。無効にしてよいのは、story が部品を単独で描画していることの副作用として鳴るものだけで、「いまは直せない」は理由にならない
