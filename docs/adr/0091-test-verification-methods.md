...
- **検査するのは撮影と同じ 1 テーマだけ**。全 story を 2 テーマぶん検査すると実行が倍になり、fork 先の private ランナー(2 コア)で課金される。撮らない側のテーマは、配色が面へ効いていることだけを見る(`vrt/theme-tokens.spec.ts`)。**片テーマでだけ出るコントラスト違反は届かない** — 配色値の正しさは token の SSOT と生成物の突き合わせ(`tokens-drift`)が持ち、ここが持つのは実描画での検出である
