...
- 基準画像は repo にコミットし、更新は**意図的な差分としてレビュー対象**にする。書き換えてよいのは**人が承認を表明したとき**に限る(PR のラベル / 手元での撮り直し)。無条件に CI が書くと、承認の実体が「CI が直した」に変わる
- **承認できる範囲は、直前の実行が報告した差分に限る**。報告した集合と撮り直す集合を同じ出所(実行結果のレポート)から取ることで、表に出ていない差分が基準画像へ入る経路を塞ぐ
...
- ❌ 人の承認なしに基準画像を書き換えること、報告されていない差分を承認の対象に含めること
