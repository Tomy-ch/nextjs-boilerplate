...
- 基準画像は repo にコミットし、更新は**意図的な差分としてレビュー対象**にする。CI は判定だけを行い、基準画像を書かない。`contents: write` を持つ job に書かせると、承認の実体が「CI が直した」に変わる
...
- ❌ 基準画像を CI に書かせること、比較対象を story 側の申告制にすること(申告制では新しく足した story が黙って対象外になる)
