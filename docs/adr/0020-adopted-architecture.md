...
- **解消**: 語彙を変えて縮退する案を採用する。安定核 `domain` は表示層の語彙 **`model`**(表示用 VO・フォーマッタ・表示バリデーション、ビジネスルール禁止)へ縮退し、`usecase`(画面ユースケース)は独立ディレクトリを持たず **feature 内へ共置**する。go との対応は下記マッピング表で担保し、層別監査・scaffold の移植グループ(GB-1 / GB-2 / GB-4)は本表を差し替えて載せる。
