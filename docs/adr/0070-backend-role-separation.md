...
- ❌ `src/` に DB 接続・ORM を足すこと([0011](0011-no-docker.md))（強制: 持たない —— 採らない決定。DB 接続・ORM は依存と接続のコードの追加として diff に現れ、置いていないこと自体が状態である）
- ❌ バックエンド API spec を手書き型で複製すること(SSOT = `openapi.gen.yaml`。生成は [0072](0072-api-type-generation.md))（強制: 散文 —— **寄せられない**。手書きの型が契約の写しかは形の一致ではなく意味で決まり、偶然同じ形の自前の view 型と区別できない）
...
- ❌ 契約が返さない値(集計値等)を表示層で計算して出すこと(規則の写しになる)（強制: 散文 —— **寄せられない**。表示層の計算が backend の規則の写しかは計算の意味で決まり、式の形からは決まらない）
