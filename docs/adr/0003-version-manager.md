...
- 通常は短縮名で宣言する。**セキュリティスキャナだけは backend (`aqua:` 等) まで明示する** — 検出そのものを担うツールであり、mise のレジストリが既定 backend を差し替えたときに取得元が黙って変わることを許さないため。他ツールへ広げるかは供給網方針の判断であり、本 ADR は現時点でそこまで課さない
