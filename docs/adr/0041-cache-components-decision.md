...
- **vendor-independent な正当性材料**([0010](0010-standards-and-non-lockin.md) §2 = Next.js の推奨だけを根拠にしない):
  ...
  - **交差関心の未確定**: 0030(A7)の env プリレンダー凍結・0080 §3.5 の Suspense × PPR 相互作用と交差する。これらを固める前に既定を反転させない。
...
- **既存 ADR 本体の相互参照の張り直しは別作業**: 本 ADR は 0040 の保留を「0.0.x = 無効」に確定するが、参照元である **0040 本文の「有効化可否は保留」/ 0071 の「0040 の保留に従う」/ 0080 §3.5 の「0040 の保留に従う」** はいずれも Accepted の Protected Documentation であり、本 ADR 作成時点では編集しない。これらへ本 ADR への back-link を付す作業は、[0010](0010-standards-and-non-lockin.md) 補足と同じく **AGENTS.md 整合 / v1 の大規模整理フェーズ**でまとめて行う。それまで旧参照は「保留に従う」の文言のまま残る(読み手が旧 ADR だけを読むと本 ADR の確定を見落とし得る点に留意)。
- 本 ADR は [0140](0140-documentation-operations.md) のタクソノミーにおいて **judgment** 分類に属する(#1 = 保留の判断を下す judgment)。
- 再評価トリガー(v1 / fork 先)で `cacheComponents: true` を採用する場合、`use cache` 粒度・PPR の static shell / dynamic hole 分割・0030 の env プリレンダー凍結との整合・0080 §3.5 の Suspense × PPR は、その時点で 0071 / 0080 のキャッシュ節を追補して確定する。
...
- [0080-error-handling.md](0080-error-handling.md)(B6)— §3.5 Suspense × PPR は 0040 保留に従う(本 ADR と連動・張り直しは補足参照)
