...
（**採番はブロック帯で確定(2026-07-14・0001〜0155(トピック順ブロック帯))**。S2 の解決を既存 ADR への追補で埋めず内容として独立させ、S ごとに 1 主題 = 1 ADR とする判断([[user]] 2026-07-14)による独立起票。内容自体はこの設計討議でユーザ確定済み。日付 2026-07-14。0.0.x の ADR は living document として本文を直接上書きする）
