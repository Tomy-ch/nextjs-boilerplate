...
- **i18n ライブラリ(next-intl 等)・ロケール解決・翻訳キー体系を 同梱しない**。用途依存のため、必要になった時点で判断する。**撤回条件は、本体が複数ロケールの文面を持つようになったとき** —— 同梱する画面のいずれかが `[locale]` セグメントか `proxy.ts` のロケール解決を実際に通るようになったときである。設置面の無い seam は腐り、採用時に必ず書き直される。**「作った側に多言語の要望があること」は条件にならない** —— 用途依存の判断は作った側が持ち、本体が持つのは採用時の座標だけである
