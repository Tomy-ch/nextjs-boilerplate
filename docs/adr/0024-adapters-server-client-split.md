...
| `adapters/client` | **`"use client"`** | `model` / `errors` / `logging` / client config(**server config 不可・secret 無**。client config = NEXT_PUBLIC リテラルは可) | 同一オリジン BFF fetch(#7)/ WebSocket・SSE(#56)/ telemetry 送信(#59 / #60)/ アップロード送信(#13。presigned 直 PUT または BFF の multipart proxy へ向けた送信。既定は backend の受け口で決まる = [0075](0075-file-upload-seam.md))/ analytics 送信(#61。**このアプリが送信を組み立てる場合に限る**。同梱のタグマネージャは組み立てを持たないため通らない = [0082](0082-client-observability.md) §3)。**remote のみ** |
