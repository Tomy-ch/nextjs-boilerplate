...
- **条件が変われば削除する**。恒久 allowlist にしない。**週に一度、宣言を条件へ突き合わせる**(`make suppression-expiry` / `.github/workflows/suppression-expiry.yaml`)。機械が決められるのは日付だけなので、**報告は全件の一覧を伴う** —— 「上流が N 以上を要求したら」のように決められない条件を黙って落とすと、条件を書いた意味が消える。見る機構が無い間に、期限を 1 か月過ぎた宣言が残っていた
