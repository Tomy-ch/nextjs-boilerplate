...
**エージェントスキルを配布するツールは審査項目が 1 つ増える**。ライブラリはビルド成果物に載るが、この種のツールは**開発者の権限で動き、何をマシン外へ送るかを自分で決める**。したがって pin の bump 時は版番号だけでなく、**CLI 表面の変化**もレビュー対象とする — 上流がサブコマンドを足せば、`.claude/settings.json` の deny が塞いでいるつもりの範囲が黙って穴になる。deny をプラットフォーム名の列挙ではなくパターンで書くのはこのためである([0154](0154-claude-skills-operations.md) 外部スキル)。
