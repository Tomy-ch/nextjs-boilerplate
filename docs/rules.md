...
- **公開 API の TSDoc は、描画される面を持つ層でだけ必須にする。** `src/components/**` は Storybook の autodocs が doc comment を描画するので、hover と Storybook の双方が読み手であり、上の問いに関わらず書く。**他層の doc comment を描画する生成器はこのリポジトリに無い**ので、上の問いに従属する —— 残余が無ければ宣言に doc comment は付かない。日本語で書き、廃止予定の API には `@deprecated` を付ける。実在する契約を述べている doc comment を、名前の言い換えだと決めつけて消さない —— **消す根拠は正本がどこに在るかを名指しできることであって、読んで不要に見えたことではない。**
- **doc comment に `@example` を置かない。** 実行されない例はコードの形をした散文で、実装が動いたあとも自分だけ古いまま残る。部品の例は Storybook が正本なので `@see` で指す。強制: **機械**（`scripts/doc-example.gate.test.ts`）—— doc comment のブロックだけを見るので、コードの中の `user@example.com` は拾わない。走査した件数そのものを主張し、0 件へ落ちたら失敗する。
...
- **変更はコメントを稼ぐのであって、連れて来ない。** 実装そのものが足すのは 0 行で、その編集が持ち込んだ制約・逸脱・契約だけを、実装のあとのコメントのパスが判定して足す（`AGENTS.md`「作業の進め方」）。指示的なコメント（「並べ替えるな」など）は削除対象にしない。**宣言を編集したら、その宣言のコメント全体を判定の対象にする** —— 自分が触った行だけではない。上の問いに落ちた文を消し、残った文は**書き直さない**。判定は冪等だが書き直しは冪等ではなく、消してから書き直す形は同じ場所を繰り返し作り直す。強制: **手順**（実装の最後に一律で走るコメントのパス）+ **散文**（`comment-reviewer`）—— 判定した結果として変わらなかったのと判定しなかったのは差分から区別できないので、判定そのものは機械へ寄せられない。
