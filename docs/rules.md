...
- **コメントを書く前に、偽になったとき何が落ちるかを問う（[0144](adr/0144-decision-enforcement-pairing.md)）。** 既に落ちるものが在るなら**書かない** —— 落ちるものが正本で、コメントはその写しとして腐るだけである。落ちるものを作れるなら**作る**（型 / テスト / 実行される例 / 生成の入力）。寄せ先の優先順は 0144 の表の「落ちる時点」が決め、型が最も早い。作れないと分かったものだけがコメントになり、行き先は下記の前提の所在テストが決める。**コメントとは、評価者を作れなかったことの記録である。**
- **残してよいのは、コードが構文的に述べられないことだけ** —— 呼び出し側の義務 / 意図的な不在 / 外の前提。コードは「自分が何をするか」しか言えないので、この 3 つはコードと競合しない。逆に**コードが述べられることをコメントが述べた瞬間、出所が 2 つになり、読み手はどちらを信じるか選ばされる**。読み手は人だけではなく、コメントを実行ロジックより優先して読む。既定は「書かない」へ倒す —— 誤ったコメントの害は、無いことの害より大きい。
- **公開 API の TSDoc は、描画される面を持つものに必須にする。** Storybook は `src/**` の story を拾い、autodocs は全 story に既定で効く（`.storybook/main.ts` / `.storybook/preview.tsx`）ので、**story を持つ部品は層を問わず** hover と Storybook の双方が読み手であり、上の問いに関わらず書く。story を持たない宣言の doc comment を描画する生成器はこのリポジトリに無いので、上の問いに従属する —— 残余が無ければ宣言に doc comment は付かない。日本語で書き、廃止予定の API には `@deprecated` を付ける。実在する契約を述べている doc comment を、名前の言い換えだと決めつけて消さない —— **消す根拠は正本がどこに在るかを名指しできることであって、読んで不要に見えたことではない。**
- **doc comment に `@example` を置かない。** 実行されない例はコードの形をした散文で、実装が動いたあとも自分だけ古いまま残る。部品の例は Storybook が正本なので `@see` で指す。強制: **機械**（`scripts/doc-example.gate.test.ts`）—— doc comment のブロックだけを見るので、コードの中の `user@example.com` は拾わない。走査したファイル数そのものを主張し、下限（`MINIMUM_SOURCES`）を割ったら失敗する。
...
- **変更はコメントを稼ぐのであって、連れて来ない。** 実装そのものが足すのは 0 行で、その編集が持ち込んだ制約・逸脱・契約だけを、実装のあとのコメントのパス（`/settle-comments`）が判定して足す（`AGENTS.md`「Task Execution Protocol」）。指示的なコメント（「並べ替えるな」など）は削除対象にしない。**宣言を編集したら、その宣言のコメント全体を判定の対象にする** —— 自分が触った行だけではない。上の問いに落ちた文を消し、残った文は**書き直さない**。判定は冪等だが書き直しは冪等ではなく、消してから書き直す形は同じ場所を繰り返し作り直す。強制: **手順**（実装の最後に一律で走るコメントのパス）+ **散文**（`comment-reviewer`）—— 判定した結果として変わらなかったのと判定しなかったのは差分から区別できないので、判定そのものは機械へ寄せられない。
...
- **サンプル破棄を生き延びる側には、題材の語彙を名前にも中身にも持たせない。** 名前は役割で付け、中身は抽象的な図形・ピクトグラムに留める。散文 —— **寄せられていない**。残留語彙の検査は `src/` と `mocks/` しか走査しない。
...
- **シェル変数を全角文字の直前に裸で置かない。** シェルが全角文字の先頭バイトを変数名の一部として食い、空へ展開したうえで壊れたバイト列を出す。`${NAME}` と囲む。散文 —— **寄せられていない**。壊れるのは表示だけで終了コードは変わらないため、検査でも人の目でも素通りする。
