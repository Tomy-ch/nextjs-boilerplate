...
- **決着して変更まで出た issue に `wontfix` を付けない。** 後から見た人が「検討されずに放置された」と読む。決定の生存を持つのは、その決定を書いた ADR の本文（採らない決定なら撤回条件つき。[0140](adr/0140-documentation-operations.md)）であって、issue のラベルではない。
