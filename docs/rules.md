...
- **変更はコメントを稼ぐのであって、連れて来ない。** 編集の既定は新しいコメント 0 行で、足すのはその編集が持ち込んだ制約・逸脱・契約だけ。指示的なコメント（「並べ替えるな」など）は削除対象にしない。**宣言を編集したら、その宣言のコメント全体を判定の対象にする** —— 自分が触った行だけではない。上の問いに落ちた文を消し、残った文は**書き直さない**。判定は冪等だが書き直しは冪等ではなく、消してから書き直す形は同じ場所を繰り返し作り直す。強制: **散文**（`comment-reviewer` / `comment-sweep`）—— 判定した結果として変わらなかったのと判定しなかったのは差分から区別できないので、機械へ寄せられない。
