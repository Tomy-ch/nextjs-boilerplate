...
- **export する宣言には TSDoc を書き、タグを次の規則で揃える。** 読み手は呼び出し地点の hover であり、story を持つ部品では Storybook の autodocs も同じ doc comment を描画する（`.storybook/main.ts` / `.storybook/preview.tsx`）。export しない宣言には求めず、上の問いに従属する。強制: **機械（未実装）** —— タグの有無は宣言の形から決まる。
  - **`@param` と `@returns` は必ず書く。** ただし `void` / `Promise<void>` を返す関数と、JSX を返す component には `@returns` を書かない —— 型の言い換えにしかならない。
