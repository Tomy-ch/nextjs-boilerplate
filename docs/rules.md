...
- **供給している既定のうち、別のバックエンド・別の組織・別の意匠であれば必然的に偽になるものは、その値を所有する README が節として持つ。** 節が書くのは既定値・変更する箇所・参照先の 3 つで、ルート [README](../README.md#導入時に見直す既定) の索引が**アンカー付きのリンク**で指す。索引へ既定値を写さない —— 値が 2 か所に住み、片方が遅れる（[0140](adr/0140-documentation-operations.md)）。新設するときの見出しは `boilerplate 導入時の変更点` とし、既に別の見出しで同じ判断を所有している節があるなら、隣に 2 本目を立てず既存の節を指す。**索引も節も立ち上げの順序を持たない** —— 順序と人手は [`get-started/`](get-started/) の持ち物で、そちらへは 1 行で渡す。強制: 節の実在と綴りは [`doc-links`](../scripts/doc-links.gate.test.ts) が見る（指す側がアンカーを書いていることがその条件）。失効する前提の語彙は `premise-lint`（`pnpm lint:md`）が見る。**どの値が「必然的に偽になる」かと、3 つが書けているかは散文 —— 寄せられない。** 既定かどうかは値の意味からしか決まらず、コードの形には出ない。
