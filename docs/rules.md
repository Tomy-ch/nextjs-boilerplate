...
- **私的な文書の節番号を、残る側へ焼き込まない。** issue の要件書 `§30`・計画の工程番号（`P<n>-<m>` の類）・受入条件の番号は、読み手が解決できない。制約は実体を自己完結で書く。**公開標準の節参照は可**（`RFC 6750 §5.2` / `OIDC Core §3.1.3.3`）—— 自己解決でき、安定していて、状況に依らない。リポジトリへチェックインされた ADR も同じ理由で可である。
