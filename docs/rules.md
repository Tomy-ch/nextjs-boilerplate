...
**各節は見出しの直前に錨（`<a id>`）を持ち、外から指すときはその錨を使う。** 見出しから導く slug は、訳したときと言い換えたときに消える（[ADR 0146](adr/0146-rule-reference-stability.md)）。

**機械へ寄せられるかどうかを、節頭の手段に載らない規約は自分で述べる。** 判定は **寄せられない** / **一部寄せられる** / **寄せられる** の 3 語だけを使い（[ADR 0144](adr/0144-decision-enforcement-pairing.md)）、その直後に理由を 1 文で持つ。**寄せられない**は原理的にコードの形から決まらないこと、**寄せられる**は検出の形は書けるが規則がまだ無いことで、両者を同じ語へ畳まない。**この 3 語の外を作らない** —— 綴りが散ると、[トレーサビリティ](traceability.md)の集計が一致で数えられなくなる。
...
- **build info を露出するときは commit SHA と build time の出所を明示し、機微な環境変数を含めない。** 外形監視の口（`/api/health`）は build info も載せず、生存以外を答えないことを `src/app/api/health/route.test.ts` が固定する。**根拠 ADR 無し** —— この内容を決めた ADR が存在しない。
...
- **必須の印は付けるが、ブラウザに送信を止めさせない。** 隠れている段の空欄はブラウザが focus できず、送信が理由も示さずに止まる。空欄の指摘は画面が出す。値を hidden input で運ぶ部品に `required` を持たせない —— constraint validation の対象外で、付けても検証されない。必須であることの表示は `RequirementBadge`、強制は Server Action と server 側の検証で行う。必須・任意の印は label の前に置き、支援技術から隠し、必須であることは control が伝える。
...
- **背面を塞ぐ overlay の中から遷移するときは、閉じる操作を同時に撃たない。** overlay は戻る操作のために履歴を 1 つ積んでおり、閉じるときにそれを戻す。client 側の遷移は取得が終わるまで URL を動かさないため、同じ操作で閉じると、その戻しが遷移を消す。**遷移が届いたこと（効いている条件・データが変わったこと）で閉じる。** 積んだ 1 件は結果で差し替える（`router.replace`）—— そのうえで積むと戻る操作が 1 度空振りする。overlay から遷移したあと、戻る操作が 1 度で遷移前の画面へ帰ることは `e2e/journeys/browse.spec.ts` が固定する。
- **clipboard 操作には成功・失敗のフィードバックを付け、権限拒否や非対応環境のフォールバックを表示する。**
- **画面は viewport を明示し、safe area、十分なタッチターゲット、hover 非依存を満たす。** safe area の余白は viewport の下端に固定する面が取り、`env()` を効かせる `viewport-fit` の宣言は画面の側が持つ（[ADR 0051](adr/0051-styling-system.md)「重なり順の帯」）。tooltip は pointer を合わせている間だけ現れるため touch と keyboard から到達できず、それだけに情報を持たせない。**タッチターゲットと hover 非依存は根拠 ADR 無し** —— 決めた ADR が存在しない（0100 は適合目標と検査の時点、0102 は対応ブラウザの行列で、どちらも触れない）。
- **アイコンは `src/components/icon.ts` から取る。** 供給元を直接 import しない。自作 SVG は `currentColor` を継承し、配置と用途を明示する。散文 —— **寄せられない**（自作 SVG の側）。配置と用途が明示されているかは描かれる文脈で決まり、要素の形からは決まらない。
...
- **部品の中身は帯（viewport）で分岐させない。** 同じ部品が広い場所にも狭い場所にも置かれる分岐はコンテナクエリで書く。散文 —— **寄せられる**。「部品では帯の variant を使わない」は静的に決まる。規則として正しいかは設計判断で、機械化は判断が付いてからにする。
- **画面の骨格は器の幅（container query）で分岐させない。** どこに何を置くか・出すか出さないかは帯で決める。散文 —— **寄せられる**。上と対の判断で、同じく設計判断が付いてから寄せる。
...
- **名前の付いた関数には TSDoc を書き、タグを次の規則で揃える。** 対象は `function f` / `const f = () =>` / メソッドで、export するかどうか・関数の内側にあるかどうかを問わない。読み手は呼び出し地点の hover だけではなく、そのファイルを直す人の hover でもある —— 内側で名前を付けた関数ほど、名前と型だけでは役割が読めない。story を持つ部品では Storybook の autodocs も同じ doc comment を描画する（`.storybook/main.ts` / `.storybook/preview.tsx`）。対象外は 3 つで、名前の無いコールバック（`map` などの引数へ直接渡す関数）、Next.js の特殊ファイル（`page` / `layout` / `route` など）が framework へ渡す export、そして**テストファイルの中の関数**である。前の 2 つは名前で呼ぶ読み手がいないため、テストは意図を `it` の日本語名が運ぶためで、テストの書き方は [0090](adr/0090-testing-strategy.md) が持つ。複雑度の閾値では線を引かない —— 閾値は出どころを言えず、境目で判断が揺れる。**`@returns` の有無は散文** —— **一部寄せられる**。型注釈を持たない関数では戻り値の種別が宣言の形から決まらず、「書かない」と決めた場合（`void` を返す関数、JSX を返す component）と見分けられない。
  ...
  - 日本語で書く。実在する契約を述べている doc comment を、名前の言い換えだと決めつけて消さない —— **消す根拠は正本がどこに在るかを名指しできることであって、読んで不要に見えたことではない。**
...
- **変更はコメントを稼ぐのであって、連れて来ない。** 実装そのものが足すのは 0 行で、その編集が持ち込んだ制約・逸脱・契約だけを、実装のあとのコメントのパス（`/settle-comments`）が判定して足す（`AGENTS.md`「Task Execution Protocol」）。指示的なコメント（「並べ替えるな」など）は削除対象にしない。**宣言を編集したら、その宣言のコメント全体を判定の対象にする** —— 自分が触った行だけではない。上の問いに落ちた文を消し、残った文は**この節の規則を満たす形になっていることを確かめ**、なっていなければその形へ直す —— 必須の枠が欠けていれば足し、契約と食い違う文は契約へ合わせる。強制: **手順**（実装の最後に一律で走るコメントのパス）と `comment-reviewer`。散文 —— **寄せられない**。判定した結果として変わらなかったのと、判定しなかったのは、差分から区別できない。
...
- **供給している既定のうち、別のバックエンド・別の組織・別の意匠であれば必然的に偽になるものは、その値を所有する README が節として持つ。** 節が書くのは既定値・変更する箇所・参照先の 3 つで、ルート [README](../README.md#導入時に見直す既定) の索引が**アンカー付きのリンク**で指す。索引へ既定値を写さない —— 値が 2 か所に住み、片方が遅れる（[0140](adr/0140-documentation-operations.md)）。新設するときの見出しは `boilerplate 導入時の変更点` とし、既に別の見出しで同じ判断を所有している節があるなら、隣に 2 本目を立てず既存の節を指す。**索引も節も立ち上げの順序を持たない** —— 順序と人手は [`get-started/`](get-started/) の持ち物で、そちらへは 1 行で渡す。強制: 節の実在と綴りは [`doc-links`](../scripts/doc-links.gate.test.ts) が見る（指す側がアンカーを書いていることがその条件）。失効する前提の語彙は `premise-lint`（`pnpm lint:md`）が見る。**指す先が破棄や剥がしで消えるなら、索引の行も同じマーカーで一緒に落とす** —— 残すと複製した先でリンク切れになる。見るのは `purge-verify` と `strip-verify` で、**いまのツリーだけを見る検査では届かない**。**どの値が「必然的に偽になる」かと、3 つが書けているかは散文 —— 寄せられない。** 既定かどうかは値の意味からしか決まらず、コードの形には出ない。
- **サンプル破棄を生き延びる側には、題材の語彙を名前にも中身にも持たせない。** 名前は役割で付け、中身は抽象的な図形・ピクトグラムに留める。散文 —— **寄せられる**。残留語彙の検査は `src/` と `mocks/` しか走査しない。
...
- **規約を上位の文書へ一般化したら、元の場所は 1 行の参照へ畳む。** 同じ判断を別の言い回しで両方に書くと、片方だけが直された日に黙って食い違う —— 書いた側には、一般化したことと元の場所を畳んだことの区別が付かない。残すのは、その層に固有の分（綴りの形式・その層だけの制約）だけである。
...
- **ソースの構造を字面で判定しない。構文木から読む。** 正規表現でコメントや import を拾う実装は、文字列リテラルとコメントの区別を持たない —— 文字列の中の `/*` から後ろの `*/` までを 1 つのコメントとみなし、その間の宣言ごと落とす。**doc comment は機械の入力でもある**ので、`@example` のように中へコードを書く規約が入った時点で、字面の走査は黙って別のものを数え始める。`typescript` は既に依存に在り、[`scripts/lib/one-to-one.ts`](../scripts/lib/one-to-one.ts) と [`scripts/lib/server-only.ts`](../scripts/lib/server-only.ts) が同じ読み方をしている。
...
- **決着して変更まで出た issue に `wontfix` を付けない。** 後から見た人が「検討されずに放置された」と読む。決定の生存を持つのは、その決定を書いた ADR の本文（採らない決定なら撤回条件つき。[0140](adr/0140-documentation-operations.md) 決定 2）であって、issue のラベルではない。
...
- **決定を改定したら、2 手で掃く。** 改定した節を読み返す方式では取り残す。**第 1 手** —— 否定した語彙を 3〜5 語決めて全文 grep する。残ってよいのは「それを持たない」と書いている側だけ。**第 2 手** —— 本文で書き換えた文を、その決定を要約している場所へ順に当てる。grep では拾えないのは「まだ真に見えるが読者を誤らせる文」で、これは意味だけが変わった語に宿る。要約先は有限である（[ADR 索引](adr/README.md) / 各 ADR の撤回条件 / この文書）。散文 —— **寄せられない**。どの語が意味を変えたかは、決定の意味からしか決まらない。
