...
| IM-26 | GB-3(spec 駆動)の採否判断 | C | P5-18 | v1 を切る直前の最終品質チェック | **採用は決定 / 生成 scaffold は不採用 / 残り 1 点は P7-3 後** |
...
- **未確定は 1 つ: spec を先に書くことを手順として強制するか。** 「先に書くほうが速いか」は未実装の画面でしか測れず、それを作るのは P7-3 の実行そのものであるため、**P7-3 の後**に判断する
- **注意**: go 側は spec 側に語彙表への登録まで足しており、残る 1 点を確定させるときは IM-48 の 3 と一体で判断する
