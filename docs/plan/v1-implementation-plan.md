...
- **期間で絞る操作はこの画面が持つ**。契約(`GET /v1/purchases`)がいま受け取るのは cursor の 2 つ(`after` / `first`)だけで、**取得済みのページに client 側で日付の条件を掛けると、条件に合う古い購入が落ちた一覧になる**。したがって先に**契約へ期間のクエリを足す**必要があり、そのうえで DatePicker を置く。P5-9 のマイページは集計の詳細を上位 10 件で打ち切り、それより古い購入をこの画面へ送っているので、**この画面に範囲選択が無いと「古い購入を見る」経路が閉じたままになる**
