...
| コマンド | 説明 | 補足 |
| --- | --- | --- |
| `make version-stamp [REF=<ref>]` | `package.json` の `version` をブランチ名の版へ書き換えます。 | `REF` 省略時は `GITHUB_REF_NAME`、それも無ければ手元の現在ブランチを読みます。`release/vX.Y.Z` / `hotfix/vX.Y.Z` 以外の ref では何もせず正常終了します。 |
