...
このスキルは**テスト**を所管し、所管するのはこのスキルだけである。どのレビュースキルもテストのレンズを持たず、このスキルは他のスキルの内側からではなく単体で呼ばれる。`/impl-review`（変更そのもの）は `AGENTS.md` の Review Phase Protocol における対等な相方であり、別々に問われ、別々に決められ、互いに委譲しない。`/settle-comments` は 3 本目の相方ではない —— どちらのレビューよりも前、実装の最後の手順としてコメントを決着させる。
