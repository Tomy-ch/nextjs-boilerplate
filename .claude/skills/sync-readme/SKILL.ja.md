...
置かれたファイルである。その言語と、隣に `*.ja.md` を置いてよいかは ADR
[0140](../../../docs/adr/0140-documentation-operations.md) が持つ —— 推測せず読む。0140 が日本語を
そこに置き、隣の `*.ja.md` を禁じている間は、したがってこのリポジトリの README には**翻訳の兄弟が無い** ——
...
- canonical README は接尾辞なしのパスの `README.md` で、その言語は ADR 0140 が持つ。同居する翻訳は無く、このスキルは作らない。
...
0140 が README の隣の `*.ja.md` を禁じている間、チェーンする先は無い。いま書いた README が
canonical そのものである。更新の結果それが生まれていないことを確かめ、canonical のみ更新した旨を
報告する。
