...
go-boilerplate 版は、フラットな `docs/decisions.md` を正式な `docs/adr/` 群へ移行するためのものだった。**本リポジトリはその段階を既に通過している** — 正式な ADR 群（`docs/adr/0001-*` … `0155-*`。トピック別ブロック帯で採番）と、Tier 0〜6 のすべての意思決定領域を枠 ID（`G` / `T` / `R` / `A` / `B` / `C` / `D`）と 選定済み / 実装済み のステータス対で追跡する生きたボード（`docs/adr/BACKLOG.md`）を既に持つ。 <!-- skill-lint-ignore -->
