...
## Step 1. push 前のローカルレビュー確認

事前チェックを通過した直後 — **本文を組み立てる前、push する前**に、push 前の `/impl-review` を実行するか確認する。ローカルレビューの判断点はここ 1 箇所である。実装者と別のモデルでローカル差分を見るので、モックテストでは出ないギャップを、変更がマシンを離れる前に拾える。自動実行はしない。
...
再開ではなくキャンセルにする理由: ローカルレビューは修正を生むことが多く、その修正は submit-pr が動く前に commit されている必要がある（Step 0 の clean tree 前提と Step 6 の push）。作業ツリーはどのみち変わるので「再開」するものが無い。修正を commit したあとの `/submit-pr` は、素通しで流れる安価な新規実行になる。
...
- **動作確認方法**: 具体的な確認手順。実際の変更内容に合わせて適応する（UI 変更なら `pnpm dev` と開く画面、本番ビルドを壊しうる変更なら `pnpm build`、それ以外は `pnpm lint:ci` / `pnpm typecheck` など）。
...
> 推奨: push 前に `/impl-review`（実装者とは別モデルの独立・敵対的レビュー）を実行しましたか？ 未実行なら一度回すと、モックテストでは出ない不具合（認証/IDOR・DI/SQL・共有スキーマ波及など）を PR 前に拾えます。
...
### pre-push hook が検証の工程である ── 先回りしない

`pre-push` は重い gate（`pnpm typecheck` / `make test-full` / `make secret-scan`）を lefthook 経由で回す。判断は hook に任せる。

念のためと言って `pnpm lint:ci` / `make test-full` を**手で先に回さない**。窓が複数開いていれば、CI が同一に回すものを再発見するために数分ホストを飽和させるだけであり、その飽和自体が無関係な gate を落とす（`repo-ops` §7）。**push が検証の工程である。**

**変更外の理由で hook が落ちたとき** — 別セッションが編集途中の未コミットファイル、同じ `coverage/` を奪い合う 2 つの Vitest、`PATH` に無いツール — その失敗は push しようとしている変更についてのものではない。`repo-ops` §7 が `--no-verify` の適用条件を持つ。どの gate がなぜ変更外なのかを報告し、判断は利用者に委ねる。黙って迂回しない。
...
- 選択肢（該当するものを提示）:
  - 「`/impl-review` を実行」（`/test-review` によるテスト観点と `/comment-sweep` による在庫の掃引を連鎖するため、1 回で 3 つを覆う） — ローカル差分対象の別モデル敵対的レビュー（認証 / IDOR / DI / SQL / 共有スキーマ波及などモックで出ない不具合に強い）。残った指摘は既定で PR へインラインコメント投稿される（`--no-comment` で抑止）
...
- [ ] タイトル・本文が日本語である
- [ ] タイトルが 70 文字以内である
- [ ] (推奨) push 前に `/impl-review` の実行を確認した（非ブロッキング）
