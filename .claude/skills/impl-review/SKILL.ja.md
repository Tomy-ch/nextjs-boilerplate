...
## Step 4.6 — コメント在庫を `/comment-sweep` へ委譲
...
触れたファイルを範囲として `/comment-sweep` を連鎖させ、あちらの範囲確認を飛ばさせる。

- `scope` —— Step 1 の非生成ソースファイル（`**/gen/**`・Markdown・deny リストは除く）

返ってきたレポートは Step 5 のレポートの 1 節として埋め込む。**`comment-reviewer` が既に挙げたものを再報告しない** —— 掃引の寄与は在庫に対する管轄の判定であって、差分についての 2 つめの意見ではない。

ソースのコメントに一切触れない変更では委譲を飛ばし、`コメント在庫:` 行にその旨を書く。黙らせない。
