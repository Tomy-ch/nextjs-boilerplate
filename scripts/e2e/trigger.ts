 ...
 *
 * **E2E で待たせる代償は、検出が 1 merge 遅れることだけではありません。** 基準画像の撮り直しは
 * 報告された差分を入力に取り、その報告は PR の実行にしか出ません（`baseline/README.md`）。PR で
 * 比較しなかった差分が merge 後に食い違うと、撮り直す相手を名指しできる PR がもう無く、release
 * ブランチが赤いまま別の PR を起こすことになります。**この非対称が、ラベル待ちにできない差分の
 * 線を決めています。**
 *
