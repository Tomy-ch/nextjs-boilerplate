...
describe("正常系", () => {
  describe("decodeContentsResponse", () => {
    ...
    it("blob SHA をそのまま取り出す", () => {
      expect(decodeContentsResponse(response()).sha).toBe(
        "aa62bff3e087b544494f012d5009702492e40d79",
      );
    });
  });
});
...
describe("異常系", () => {
  describe("decodeContentsResponse", () => {
    ...
    it("本文を持たない応答を拒否する", () => {
      expect(() => decodeContentsResponse(response({ content: undefined }))).toThrow(
        "本文が空です",
      );
    });
    it("空文字列の本文を拒否する", () => {
