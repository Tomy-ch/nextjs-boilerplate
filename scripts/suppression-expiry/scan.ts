...
function osvSuppressions(): readonly Suppression[] {
  return [...read(OSV_PATH).matchAll(OSV_ENTRY)].map(([, subject, condition]) => ({
    source: OSV_PATH,
    subject: subject ?? "",
    condition: condition ?? "",
  }));
 ...
 */
function cooldownSuppressions(): readonly Suppression[] {
  const lines = read(WORKSPACE_PATH).split("\n");
...
function zapSuppressions(): readonly Suppression[] {
  return read(ZAP_PATH)
    .split("\n")
...
/** 3 つの面の宣言を 1 つの一覧へ均す。 */
export function scanSuppressions(): readonly Suppression[] {
  return [...osvSuppressions(), ...cooldownSuppressions(), ...zapSuppressions()];
}
